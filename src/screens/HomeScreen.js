...
export default function HomeScreen({ navigation }) {
  ...
  const filteredIssues = selectedCategory === 'all'
    ? issues.filter(issue => issue.is_popular || issue.isPopular)
    ...
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        {user && (
          <View style={styles.wallet}>
            ...
            <Text style={styles.walletAmount}>{formatNumber(user.coins)}</Text>
          </View>
        )}
      </View>
      ...
      >
        <Text style={styles.sectionTitle}>
          {selectedCategory === 'all' ? '인기 예측' : `${selectedCategory} 예측`}
        </Text>

        {loading ? (
          <Text style={styles.loadingText}>로딩 중...</Text>
        ) : filteredIssues.length === 0 ? (
          <Text style={styles.emptyText}>예측 이슈가 없습니다.</Text>
        ) : (
